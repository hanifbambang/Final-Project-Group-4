...
CashPayment processCashPayment(float totalBill) {
    CashPayment payment;
    payment.totalBill = totalBill;
    
    printf("\nTotal Bill: %.2f\n", totalBill);
    printf("Enter Cash Amount: ");
    scanf("%f", &payment.cashReceived);
    
    if (payment.cashReceived >= totalBill) {
        payment.change = payment.cashReceived - totalBill;
...
float hospitalCheckout() {
    float consultationFee = 50.0;
    float medicineCost;
    float totalBill;
    
    printf("\n--- Hospital Checkout Simulation ---\n");
    printf("Consultation Fee: %.2f\n", consultationFee);
    
    printf("Enter Medicine Cost: ");
    scanf("%f", &medicineCost);
    
    totalBill = consultationFee + medicineCost;
        ...
        switch (Roless)
        {
            ...
            case PATIENT:
                ...
                if ((strcmp(username, "admin") == 0) && (strcmp(password, "01234") == 0))
                {
                    ...
                    case 1: 
                        {
                            float totalBill = hospitalCheckout();
                            CashPayment payment = processCashPayment(totalBill);
                            break;
                        }
